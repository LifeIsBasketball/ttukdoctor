...
export const GarageCard:React.FC<Props> = ({className="", id="", onClick, title, distance, address, rating=4, hashTag=[''],reviewCount=0, imgSrc, cost= -1}) => {
    return (
        ...
        onClick={onClick}  style={{padding: '8px', width: '100%', height: 64, position: 'relative', background: 'white', overflow: 'hidden', borderRadius: 12}}>
            <div style={{display: 'flex', flexDirection: 'row', justifyContent:'space-between'}}>
                <div style={{padding: '4px 0px', display:'flex', flexDirection: 'column',gap: '2px', textAlign: 'left', justifyContent: 'center'}}>
                    ...
                    <div style={{display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '2px'}}>
                        ...
                        <div style={{display: 'flex', alignItems: 'center'}}>
                          ...
                          {/*이거 이정명이 일단 하드코딩 해놨습니다 리뷰 0/0으로! */}
                ...
                {cost== -1 && <img style={{width: 48, height: 48, borderRadius: 8}} src={imgSrc} />}
                {cost!= -1 && (
                  <>
                    <div style={{position: 'relative', top: '0px'}}>
                      <RightArrowButton size={20} />
                    </div>
                    <div style={{position: 'absolute', bottom: '8px', right: '8px', textAlign: 'right', color: 'black', fontSize: 14, fontFamily: 'Noto Sans KR', fontWeight: '700', wordWrap: 'break-word'}}>{cost.toLocaleString("ko-KR") + "원"}</div>
                  </>
                )}
            </div>
